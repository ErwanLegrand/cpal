...
fn format_support_from_hresult(hr: HRESULT) -> Result<bool, Error> {
    match hr {
        ...
        // use of this endpoint off, or another application already holds the device exclusively.
        // Reporting these as "unsupported" would empty out `supported_*_configs_with(Exclusive)`,
        // which probes formats one by one with no other check in front of it, and turn an
        // unplugged, busy, or exclusive-denied device into `UnsupportedConfig`. Propagated, they
        // map to DeviceNotAvailable / StreamInvalidated / HostUnavailable / ExclusiveModeDenied /
        // DeviceBusy in `impl From<windows::core::Error> for Error`.
...
/// Sample formats probed against the endpoint in exclusive mode, ordered by preference (most
/// preferred first, per `cmp_default_heuristics`).
...
mod tests {
    ...
    fn the_exclusive_probe_order_agrees_with_cmp_default_heuristics() {
        for pair in EXCLUSIVE_SAMPLE_FORMATS.windows(2) {
    ...
    #[test]
    fn the_exclusive_probe_set_matches_supported_formats() {
        let mut probed = EXCLUSIVE_SAMPLE_FORMATS;
        probed.sort_unstable();

        let mut expected = WAVEFORMATEXTENSIBLE_SAMPLE_FORMATS;
