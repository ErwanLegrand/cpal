...
pub unsafe fn is_format_supported(
    ...
    //
    // Shared mode also has S_FALSE (hr.0 == 1), meaning the engine proposed a different, closest
    // format; that is only usable when AUTOCONVERTPCM is set (output). Exclusive mode never
    // returns S_FALSE — it reports AUDCLNT_E_UNSUPPORTED_FORMAT instead, and there is no
    // AUTOCONVERTPCM fallback to reach for — so `hr.0 == 0` is the right test in both modes,
...
impl Device {
    ...
    unsafe fn initialize_audio_client(
        ...
        let aligned_frames = audio_client
            .GetBufferSize()
            .context("Failed to get aligned buffer size")?;
...
//
// `GetMixFormat` describes the shared-mode engine, so in exclusive mode it says nothing about
// what the endpoint accepts; the device is asked directly instead. I24 is in the list because
// 24-bit-in-a-32-bit-container is what a great many audio interfaces actually run natively,
// and omitting it would leave those devices with no exclusive-mode default at all.
...
unsafe fn exclusive_default_format(
    ...
    for sample_format in EXCLUSIVE_SAMPLE_FORMATS {
        ...
        let format_ptr = &waveformat.Format as *const Audio::WAVEFORMATEX;
        if is_format_supported(client, ShareMode::Exclusive, format_ptr)? {
            return Ok(format_from_waveformatex_ptr(format_ptr, client));
        }
    }
...
///
/// Shared mode keeps CPAL's existing behaviour exactly: a fixed buffer size is converted to a
/// duration and `BufferSize::Default` becomes 0, which asks the engine for its default period.
/// Exclusive mode cannot use 0 — the same value is also the periodicity, and a zero periodicity
/// is rejected — so `BufferSize::Default` resolves to the device's minimum period, the
/// lowest-latency value the endpoint will accept.
